...
class TestProviderRegistry:
    ...
    def test_create_strategy_from_instance_requires_config_aware_factory(self):
        """Instance strategy factories must accept the config argument passed by the registry."""
        # This mirrors the provider/DI wiring bug: DI registration stores a zero-arg
        # closure, but the registry always invokes strategy factories as factory(config).
        self.registry.register_provider_instance(
            provider_type="azure",
            instance_name="azure-primary",
