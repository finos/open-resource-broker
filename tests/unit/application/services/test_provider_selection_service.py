...
class TestProviderSelectionService:
    ...
    def mock_config_manager(self):
        ...
        config_manager.get_active_provider_override.return_value = None
    ...
    def test_explicit_provider_type_alias_resolves_single_enabled_instance(
        self, mock_logger
    ):
        config_manager = Mock(spec=ConfigurationManager)
        provider_config = ProviderConfig(
        ...
        config_manager.get_provider_config.return_value = provider_config
        config_manager.get_active_provider_override.return_value = None
        service = ProviderSelectionService(config_manager, mock_logger)

        template = Template(
            template_id="azure-alias-test",
            provider_name="azure",
            provider_api="VMSS",
            image_id="img",
            subnet_ids=["subnet-123"],
        ...
        assert result.provider_instance == "azure-default"

    def test_explicit_provider_type_alias_does_not_resolve_if_ambiguous(
        self, mock_logger
    ):
        config_manager = Mock(spec=ConfigurationManager)
        provider_config = ProviderConfig(
        ...
        template = Template(
            template_id="azure-ambiguous-alias-test",
            provider_name="azure",
            provider_api="VMSS",
            image_id="img",
            subnet_ids=["subnet-123"],
        ...
        with pytest.raises(ValueError, match="Provider instance 'azure' not found"):
            service.select_provider_for_template(template)

    def test_override_provider_type_alias_resolves_to_instance_name(self, mock_logger):
        config_manager = Mock(spec=ConfigurationManager)
        provider_config = ProviderConfig(
            selection_policy="FIRST_AVAILABLE",
            providers=[
        ...
        config_manager.get_provider_config.return_value = provider_config
        config_manager.get_active_provider_override.return_value = "azure"
        service = ProviderSelectionService(config_manager, mock_logger)

        template = Template(
            template_id="azure-override-alias-test",
            provider_api="VMSS",
            image_id="img",
            subnet_ids=["subnet-123"],
            max_instances=1,
    ...
    def test_template_loading_override_provider_type_alias_resolves_to_instance_name(
        self, mock_logger
    ):
        config_manager = Mock(spec=ConfigurationManager)
        provider_config = ProviderConfig(
        ...
        result = service.select_provider_for_template_loading()
