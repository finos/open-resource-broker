...
class TestProviderCapabilityService:
    ...
    def test_registry_backed_capabilities_override_stale_azure_fallback(self, mock_logger):
        """Registry-backed Azure capabilities should win over the hardcoded fallback block."""
        mock_strategy = Mock()
        mock_strategy.get_capabilities.return_value = ProviderCapabilities(
            provider_type="azure",
            ...
            features={
                "supported_apis": ["VMSS", "VMSSUniform", "SingleVM", "CycleCloud"],
                "api_capabilities": {
                    "CycleCloud": {
        ...
        service = ProviderCapabilityService(mock_logger, provider_registry=mock_registry)

        capabilities = service.get_provider_api_capabilities("azure-default", "CycleCloud")

        assert capabilities["required_create_fields"] == ["cluster_name", "node_array"]
        assert capabilities["supports_async_operations"] is True
        mock_registry.create_strategy_from_instance.assert_called_once_with("azure-default", {})
