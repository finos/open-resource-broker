"""Architecture test: ``kubernetes`` must not be imported outside the kubernetes
provider tree.

Importing the ``kubernetes`` SDK directly in core or infrastructure modules
couples the entire application to the ``[kubernetes]`` (or ``[k8s-legacy]``)
extra even when neither is installed.  All ``kubernetes`` SDK usage must be
confined to:

* ``src/orb/providers/kubernetes/`` — modern provider (TYPE-CHECKED)
* ``src/orb/k8s_legacy/``           — legacy plugin (maintenance mode)
...
_PROVIDERS_KUBERNETES_DIR = SRC_ORB / "providers" / "kubernetes"
_K8S_LEGACY_DIR = SRC_ORB / "k8s_legacy"

# All source files that live outside the kubernetes provider tree and the
# legacy k8s plugin tree.
_NON_KUBERNETES_FILES = [
    f
    for f in collect_python_files(SRC_ORB)
    if not f.is_relative_to(_PROVIDERS_KUBERNETES_DIR)
    and not f.is_relative_to(_K8S_LEGACY_DIR)
...
def test_no_kubernetes_outside_provider(filepath: Path) -> None:
    """``kubernetes`` imports must not appear outside the kubernetes provider trees."""
    ...
    assert new_violations == [], (
        f"{rel} imports kubernetes outside providers/kubernetes/ or k8s_legacy/ — "
        f"move to providers/kubernetes/ or guard with try/except ImportError. "
