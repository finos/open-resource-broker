...
from orb.providers.kubernetes.reconciliation.orphan_gc import (
...
def _make_gc(
    *,
    ...
    config: KubernetesProviderConfig | None = None,
    ...
    cfg = config or KubernetesProviderConfig(namespace="orb")
...
async def test_default_config_logs_orphans_but_does_not_delete() -> None:
    cfg = KubernetesProviderConfig(namespace="orb")  # auto_cleanup_orphans=False default
...
async def test_auto_cleanup_true_deletes_orphans() -> None:
    cfg = KubernetesProviderConfig(namespace="orb", auto_cleanup_orphans=True)
    pods = [
        _pod(name="orph-a", request_id="r-stranger"),
        _pod(name="orph-b", request_id=None),
    ]
