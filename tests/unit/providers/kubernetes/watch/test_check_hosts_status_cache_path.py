"""Cache-first read path for :meth:`KubernetesPodHandler.check_hosts_status`.
...
) -> tuple[KubernetesPodHandler, MagicMock]:
    client = MagicMock()
    core_v1 = MagicMock()
...
def test_cache_hit_skips_list_call() -> None:
    ...
    # Per-instance fields should match the handler's list-path shape.
...
def test_cache_disabled_falls_back_to_list() -> None:
    ...
    cache = PodStateCache()
    request = _make_request(requested_count=2)
    # Seed entries that would otherwise satisfy the read.
    _seed_cache_for(cache, request, count=2, status="running")
...
def test_stale_entries_are_dropped_and_fall_back() -> None:
    ...
    request = _make_request(requested_count=2)
    _seed_cache_for(cache, request, count=2, status="running")
    # Make sure enough wall-clock time has elapsed to be deemed stale.
    time.sleep(0.02)
    ...
    # Stale entries dropped -> get() returns None -> list path.
    core_v1.list_namespaced_pod.assert_called_once()
