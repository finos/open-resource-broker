"""Unit tests for :class:`KubernetesWatcher`.
...
def test_is_resource_version_too_old_detects_410() -> None:
    ...
    assert KubernetesWatcher._is_resource_version_too_old(ApiException(status=410))
    assert not KubernetesWatcher._is_resource_version_too_old(ApiException(status=500))
    assert not KubernetesWatcher._is_resource_version_too_old(RuntimeError("oops"))
