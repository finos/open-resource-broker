"""Unit tests for :mod:`orb.providers.kubernetes.utilities.deployment_spec`.
...
from orb.providers.kubernetes.utilities.deployment_spec import (
    build_deployment_spec,
    make_deployment_name,
)
from orb.providers.kubernetes.utilities.pod_spec import LEGACY_REQUEST_ID_LABEL
...
def _build_template(**provider_data) -> Template:
    return Template(
        ...
        max_instances=5,
        provider_data={"kubernetes": dict(provider_data)} if provider_data else {},
    )
...
def test_build_deployment_spec_replicas_and_selector() -> None:
    ...
    assert match_labels["orb.io/provider-api"] == "KubernetesDeployment"
...
def test_build_deployment_spec_pod_template_labels_and_restart_policy() -> None:
    ...
    assert labels["orb.io/provider-api"] == "KubernetesDeployment"
...
def test_build_deployment_spec_respects_custom_label_prefix() -> None:
    request = _build_request()
    template = _build_template()
    config = KubernetesProviderConfig(label_prefix="example.com", emit_legacy_labels=False)
    dep = build_deployment_spec(
        template,
        request,
