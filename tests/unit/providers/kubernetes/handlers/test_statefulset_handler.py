"""Unit tests for :class:`KubernetesStatefulSetHandler` (Phase E).
...
def _make_handler(client: Any | None = None) -> KubernetesStatefulSetHandler:
    if client is None:
        client = _make_client()
    config = KubernetesProviderConfig(namespace="orb-test")
    return KubernetesStatefulSetHandler(
...
def test_get_example_templates_returns_statefulset_example() -> None:
    examples = KubernetesStatefulSetHandler.get_example_templates()
    assert len(examples) >= 1
    example = examples[0]
    assert example.provider_api == "KubernetesStatefulSet"
...
def test_provider_api_key_matches_value_object() -> None:
    ...
    assert KubernetesStatefulSetHandler.PROVIDER_API == KubernetesProviderApi.STATEFUL_SET.value
