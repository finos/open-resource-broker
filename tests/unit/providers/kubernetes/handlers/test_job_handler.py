"""Unit tests for :class:`KubernetesJobHandler` (Phase E).
...
def _make_handler(client: Any | None = None) -> KubernetesJobHandler:
    if client is None:
        client = _make_client()
    config = KubernetesProviderConfig(namespace="orb-test")
    return KubernetesJobHandler(
...
def test_get_example_templates_returns_job_example() -> None:
    examples = KubernetesJobHandler.get_example_templates()
    assert len(examples) >= 1
    example = examples[0]
    assert example.provider_api == "KubernetesJob"
...
def test_provider_api_key_matches_value_object() -> None:
    ...
    assert KubernetesJobHandler.PROVIDER_API == KubernetesProviderApi.JOB.value
