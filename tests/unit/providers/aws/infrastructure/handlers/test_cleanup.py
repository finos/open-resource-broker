...
def _make_config_port(enabled=True, delete_lt=True, dry_run=False, asg=True, ec2_fleet=True, spot_fleet=True):
    port = MagicMock()
...
class TestCleanupConfigSchema:
    ...
    def test_per_resource_toggle(self):
        cfg = CleanupConfig(resources=CleanupResourcesConfig(asg=False, ec2_fleet=True, spot_fleet=False))
...
class TestASGCleanupOnReturn:
    ...
    def test_full_return_triggers_lt_cleanup(self):
        ...
        with patch.object(handler, "_delete_asg") as mock_delete_asg, \
    ...
    def test_prefix_stripped_from_asg_name(self):
        ...
        with patch.object(handler, "_delete_asg"), \
             patch.object(handler, "_delete_orb_launch_template") as mock_delete_lt:
            handler._release_hosts_for_single_asg(
                "orb-asg-req-abc-123",
                ["i-1"],
...
class TestEC2FleetCleanupOnReturn:
    def _fleet_details(self, fleet_type="maintain", total=2, tags=None):
        resolved_tags = [{"Key": "orb:request-id", "Value": "req-fleet-abc"}] if tags is None else tags
        return {
            "Type": fleet_type,
...
class TestSpotFleetCleanupOnReturn:
    def _fleet_details(self, fleet_type="maintain", target=2, tags=None):
        resolved_tags = [{"Key": "orb:request-id", "Value": "req-spot-abc"}] if tags is None else tags
        return {
    ...
    def test_full_return_triggers_lt_cleanup(self):
        ...
        with patch.object(handler._release_manager, "_retry", create=True), \
             patch.object(handler, "_delete_orb_launch_template") as mock_delete_lt:
