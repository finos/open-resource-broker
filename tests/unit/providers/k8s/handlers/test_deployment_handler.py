"""Unit tests for :class:`K8sDeploymentHandler` (Phase D).
...
def _make_handler(client: Any | None = None) -> K8sDeploymentHandler:
    if client is None:
        client = _make_client()
...
def test_get_example_templates_returns_deployment_example() -> None:
    examples = K8sDeploymentHandler.get_example_templates()
    assert len(examples) >= 1
    example = examples[0]
    assert example.provider_api == "Deployment"
...
def test_provider_api_key_matches_value_object() -> None:
    ...
    assert K8sDeploymentHandler.PROVIDER_API == KubernetesProviderApi.DEPLOYMENT.value
