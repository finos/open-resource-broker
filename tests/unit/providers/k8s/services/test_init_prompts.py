...
class TestDiscoverInfrastructureInteractive:
    ...
    def test_rbac_all_granted_no_abort_prompt(self) -> None:
        ...
        # Inputs: confirm=<enter>, context=<enter>, namespace=<enter>, sa=<enter>
        # pull_secrets is empty so no secret prompt; RBAC granted so no Continue? prompt
        with patch("builtins.input", side_effect=self._inputs("", "", "", "")):
            result = svc.discover_infrastructure_interactive({})
    ...
    def test_rbac_failure_abort_raises(self) -> None:
        ...
        # Inputs: confirm=<enter>, context=<enter>, namespace=<enter>, sa=<enter>,
        #         rbac-continue=n  (no pull-secret prompt — empty list)
    ...
    def test_rbac_failure_continue_does_not_raise(self) -> None:
        ...
        # Inputs: confirm=<enter>, context=<enter>, namespace=<enter>, sa=<enter>,
        #         rbac-continue=y  (no pull-secret prompt — empty list)
        with patch("builtins.input", side_effect=self._inputs("", "", "", "", "y")):
            result = svc.discover_infrastructure_interactive({})
    ...
    def test_chosen_sa_stored_in_result(self) -> None:
        ...
        # Inputs: confirm=<enter>, context=<enter>, namespace=<enter>, sa=select "2"=orb-runner
        # no pull-secret prompt (empty list); RBAC all granted → no continue prompt
        with patch("builtins.input", side_effect=self._inputs("", "", "", "2")):
            result = svc.discover_infrastructure_interactive({})
    ...
    def test_rbac_probe_exception_produces_all_false(self) -> None:
        ...
        # Inputs: confirm=<enter>, context=<enter>, namespace=<enter>, sa=<enter>,
        #         rbac-continue=y  (RBAC exception → all-False → Continue? prompt)
        with patch("builtins.input", side_effect=self._inputs("", "", "", "", "y")):
            result = svc.discover_infrastructure_interactive({})
