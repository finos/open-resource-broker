...
def _run_interactive(
    ...
    *,
    ...
    answers = iter([in_cluster_confirm, context_pick, namespace_pick, sa_pick, secret_pick])
    with patch(
        "orb.providers.k8s.services.init_prompts.input",
        side_effect=lambda _prompt: next(answers, ""),
    ):
        return svc.discover_infrastructure_interactive({})
...
class TestRbacProbeDisplayedNotInReturn:
    ...
    def test_rbac_denied_shows_prompt_and_aborts_on_no(self) -> None:
        ...
        # Answers: in-cluster=n, context=1, namespace=1, sa=1, secret=1, rbac-continue=N
        answers = iter(["n", "1", "1", "1", "1", "N"])
        with patch(
            "orb.providers.k8s.services.init_prompts.input",
            side_effect=lambda _p: next(answers, ""),
        ):
