"""Unit tests for the five k8s provider strategy fixes.

Covers:
1. fulfillment_final promotion on synchronous Accepted acquires
2. DESCRIBE_RESOURCE_INSTANCES routing and capability advertisement
3. dry_run context propagation
4. Synthetic instance_type derived from provider_api
5. _API_ALIASES for lowercase provider_api variants
"""
...
class TestDescribeResourceInstancesRouting:
    ...
    @pytest.mark.asyncio
    async def test_describe_surfaces_provider_fulfilment_in_metadata(self) -> None:
        mock_handler = MagicMock()
        mock_handler.check_hosts_status = MagicMock(
            return_value=_make_check_result(
                instances=[{"instance_id": "pod-0", "status": "running"}],
        ...
        result = await strategy.execute_operation(operation)
        assert result.success is True
        assert result.metadata.get("provider_fulfilment") == "fulfilled"
