...
class TestCircularDependencyFix:
    ...
    def test_provider_capability_service_gets_shared_provider_registry(self, container):
        """Capability validation should use the shared provider registry, not fallback metadata."""
        register_core_services(container)
        register_provider_services(container)

        capability_service = container.get(ProviderCapabilityService)

        assert capability_service._provider_registry is get_provider_registry()
