"""End-to-end integration test for the ``KubernetesDeployment`` API.
...
from tests.integration.providers.kubernetes.conftest import (
...
def _build_deployment_request(*, namespace: str, requested_count: int) -> Any:
    template = make_template(provider_api="KubernetesDeployment", namespace=namespace)
    return make_request(
...
async def test_deployment_full_lifecycle_acquire_status_selective_release() -> None:
    ...
    assert body.metadata.labels["orb.io/provider-api"] == "KubernetesDeployment"
