...
* a :class:`KubernetesProviderStrategy` configured with the per-test
  namespace ``orb-it`` and a ``MagicMock`` ``KubernetesClient`` whose
...
def make_namespaced_config(
    *,
    ...
    ``test_watch_lifecycle.py`` directly via :class:`KubernetesWatcher`.
    """
...
def make_template(
    *,
    ...
    return Template(
        ...
        provider_data={"kubernetes": kubernetes_block},
    )
...
def make_pod_object(
    *,
    ...
    labels: dict[str, str] = {
        ...
        f"{label_prefix}/provider-type": "kubernetes",
    }
    if extra_labels:
...
def make_kubernetes_client_mock(
    *,
    ...
    """Return a mock :class:`KubernetesClient` with the three API facets populated."""
    client = MagicMock(name="KubernetesClient")
...
def make_strategy(
    *,
    ...
    """Build a fully wired :class:`KubernetesProviderStrategy` for tests.
...
@pytest.fixture
def k8s_config(k8s_namespace: str) -> KubernetesProviderConfig:
