"""End-to-end integration test for the ``KubernetesStatefulSet`` API.
...
def _build_sts_request(*, namespace: str, requested_count: int) -> Any:
    template = make_template(provider_api="KubernetesStatefulSet", namespace=namespace)
    return make_request(
...
async def test_statefulset_full_lifecycle_acquire_status_selective_release() -> None:
    ...
    assert body.metadata.labels["orb.io/provider-api"] == "KubernetesStatefulSet"
