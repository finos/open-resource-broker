...
class TestDiscoverInteractiveSmokeTest:
    ...
    def test_interactive_happy_path_returns_expected_shape(self) -> None:
        """Interactive discovery returns the same dict shape as non-interactive."""
        console = FakeConsoleAdapter()
        svc = self._build_svc_with_leaf_mocks(console)

        # Script: confirm out-of-cluster=n (stay out), pick context 1, namespace 1, SA skip, pull secret n, RBAC already granted
        scripted_inputs = iter(["n", "1", "1", "", "", ""])
    ...
    def test_interactive_rbac_failure_continue(self) -> None:
        ...
        # Script: accept out-of-cluster, pick ctx 1, ns 1, skip SA, skip pull-secret, continue=y
        scripted_inputs = iter(["", "1", "1", "", "", "y"])
        with patch("builtins.input", side_effect=lambda _prompt="": next(scripted_inputs)):
    ...
    def test_interactive_403_namespace_fallback_skips_prompt(self) -> None:
        ...
        # Script: accept out-of-cluster, context 1, skip SA, skip pull-secret, continue with all RBAC granted
        scripted_inputs = iter(["", "1", "", "", ""])
        with (
