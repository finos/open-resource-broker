...
from tests.providers.aws.live import plugin_io_schemas
from tests.providers.aws.live.cleanup_helpers import cleanup_tracked_requests
from tests.providers.aws.live.parse_output import parse_and_print_output
from tests.providers.aws.live.template_processor import TemplateProcessor
from tests.providers.aws.live.test_onaws import get_instances_states
...
def _get_boto_profile_and_region() -> tuple[str | None, str]:
    """Read AWS profile and region from ORB_CONFIG_DIR config, same as test_onaws._get_boto_clients."""
...
def get_ec2_fleet_instances(fleet_id: str) -> List[str]:
    ...
    try:
        response = _get_ec2_client().describe_fleet_instances(FleetId=fleet_id)
...
def verify_ec2_fleet_instances_detached(
    ...
    try:
        ...
        for fleet in response.get("Fleets", []):
            ...
            try:
                fleet_instances = _get_ec2_client().describe_fleet_instances(FleetId=fleet_id)
                active_instance_ids = [
...
@pytest.fixture
def setup_multi_ec2_fleet_templates(test_session_id):
...
def test_multi_ec2_fleet_termination(setup_multi_ec2_fleet_templates):
    ...
    for request_id, template_id in request_ids:
        ...
        # Verify instances are provisioned (tolerant of AWS capacity shortfall;
        # assert_terminal_ok above already enforced "some capacity delivered").
