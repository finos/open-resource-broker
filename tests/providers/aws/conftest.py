"""Provider-level fixtures for the AWS test tree.

Covers both moto (mocked) and live subtrees.  The fixtures that call real
AWS are gated on the --live flag so they are never accidentally executed in
CI.

This file is the canonical source of:
  - moto_aws context manager
  - moto_vpc_resources
  - orb_config_dir (moto-backed)
  - Live-test session fixtures (gated by --live)
...
def make_patch_moto_compat():
    """Return a context manager that patches moto-incompatible behaviours.

    Extracted as a helper so it can be reused from the moto/ conftest.
    """
...
# ---------------------------------------------------------------------------
# ORB config directory (moto-backed)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Handler factory helpers (shared across moto/ and contract/ subtrees)
# ---------------------------------------------------------------------------
