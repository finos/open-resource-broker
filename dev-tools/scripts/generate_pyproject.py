        ...
        for key, value in data.items():
            if isinstance(value, dict):
                ...
                if all(isinstance(item, str) for item in value):
                    ...
                    lines.append(f"{prefix}{key} = [")
                    for item in value:
                        lines.append(f"{prefix}    {{" + ", ".join(f'{k} = "{v}"' for k, v in item.items()) + "},")
                    lines.append(f"{prefix}]")
    ...
    while i < len(lines):
        ...
        if line.startswith('[project]'):
            ...
            for py_version in python_versions:
                if py_version.strip():
                    new_lines.append(f'    "Programming Language :: Python :: {py_version.strip()}",')
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Generate pyproject.toml with selective templating")
