...
|------------------------|-------------------|----------------------------------------------------------|
| `KubernetesPod`        | bare `v1/Pod`     | Stateless short-lived workers; smallest blast radius.    |
| `KubernetesDeployment` | `apps/v1/Deployment` | Long-running stateless services; replica-driven scaling. |
| `KubernetesStatefulSet`| `apps/v1/StatefulSet` | Workloads needing stable network identity / storage. |
| `KubernetesJob`        | `batch/v1/Job`    | Run-to-completion batches.                               |
...
* [Configuration reference](configuration.md) - every `KubernetesProviderConfig` field.
