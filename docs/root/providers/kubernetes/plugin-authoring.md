...
| Goal                                                         | Right approach                                              |
|--------------------------------------------------------------|-------------------------------------------------------------|
| Add a workload shape that maps to a CRD (MPIJob, RayCluster, etc.) | Pattern A - subclass `KubernetesProviderStrategy`, add a new handler. |
...
* `KubernetesProviderStrategy._HANDLERS` (via the
  `register_kubernetes_handler` helper introduced below) - to attach a
...
Subclasses the standard KubernetesHandlerBase so it inherits client
...
class KubernetesMPIJobHandler(KubernetesHandlerBase):
    ...
    async def acquire_hosts(
        ...
        api = self.client.custom_objects_api  # provided by KubernetesClient
        self.with_retry(
    ...
    async def release_hosts(
        ...
        # cascade-delete pods.
        namespace = (
            (request.provider_data or {}).get("kubernetes", {}).get("namespace")
            or self._config.namespace
        )
        mpijob_name = (
            (request.provider_data or {}).get("kubernetes", {}).get("mpijob_name")
        )
        if not mpijob_name:
            return
...
```toml
dependencies = [
  "orb-py[kubernetes]>=1.7,<2.0",
]
```

The plugin contract - `register()`, `KubernetesHandlerBase`,
`KubernetesProviderStrategy.register_handler`, the
