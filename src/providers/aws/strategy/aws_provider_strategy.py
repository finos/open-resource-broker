...
from providers.aws.infrastructure.services.spot_placement_score_adapter import (
...
class AWSProviderStrategy(ProviderStrategy):
    ...
    @staticmethod
    def _should_use_spot_placement(template: Any) -> bool:
    ...
    def _clone_template_for_plan_entry(
        ...
        selected_weight = (aws_template.instance_types or {}).get(selected_instance_type, 1)
        cloned_data["instance_type"] = selected_instance_type
        cloned_data["instance_types"] = {selected_instance_type: selected_weight}
    ...
    async def _handle_create_instances(self, operation: ProviderOperation) -> ProviderResult:
        ...
        try:
            ...
            if self._should_use_spot_placement(aws_template) and not bool(
                operation.context and operation.context.get("dry_run", False)
            ):
    ...
    def _validate_aws_template(self, template_config: dict[str, Any]) -> dict[str, Any]:
        ...
        if template_config.get("allocation_strategy") == "spotPlacementScore":
            if template_config.get("price_type") != "spot":
                validation_errors.append("spotPlacementScore requires price_type='spot'")

            candidate_types = list((template_config.get("instance_types") or {}).keys())
            if template_config.get("instance_type") and template_config["instance_type"] not in candidate_types:
                candidate_types.insert(0, template_config["instance_type"])
            if len(candidate_types) < 2:
                validation_errors.append(
                    "spotPlacementScore requires at least two candidate instance types"
                )
