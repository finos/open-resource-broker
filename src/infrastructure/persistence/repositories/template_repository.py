...
class TemplateSerializer:
    ...
    def to_dict(self, template: Template) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to serialize template {template.template_id}: {e}")
            raise
        ...
        try:
            ...
            if self.defaults_service:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to apply defaults, using original data: {e}")
                    processed_data = data
            ...
            template_id = processed_data.get(
                "templateId", processed_data.get("template_id"))
            if not template_id:
                raise ValueError(
                    f"No template_id found in data: {list(processed_data.keys())}")
            ...
            self.logger.debug(
                f"Converted template_data keys: {list(template_data.keys())}")
...
class TemplateRepositoryImpl(TemplateRepositoryInterface):
    ...
    def find_by_template_id(self, template_id: str) -> Optional[Template]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to find template by template_id {template_id}: {e}")
            raise
    ...
    def find_by_provider_api(self, provider_api: str) -> List[Template]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to find templates by provider_api {provider_api}: {e}")
            raise
    ...
    def search_templates(self, criteria: Dict[str, Any]) -> List[Template]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to search templates with criteria {criteria}: {e}")
            raise
