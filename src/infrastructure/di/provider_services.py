...
def _register_provider_instance_with_di(provider_instance, container: DIContainer) -> bool:
    ...
    try:
        ...
        if provider_type == "aws":
            ...
            return _register_azure_provider_with_di(provider_instance, container)
        else:
...
def _register_azure_provider_with_di(provider_instance, container: DIContainer) -> bool:
    ...
    try:
        ...
        return register_azure_provider_with_di(provider_instance, container)
    except Exception as e:
        logger.error("Failed to register Azure provider '%s': %s", provider_instance.name, str(e))
        return False
...
def _validate_provider_config(provider_config) -> bool:
    ...
    try:
        ...
        for provider_instance in provider_config.providers:
            ...
            supported_types = ["aws", "azure"]  # Add more as they're implemented
...
def _register_provider_instance(provider_instance) -> bool:
    ...
    try:
        ...
        if provider_instance.type == "aws":
            ...
            return True
        elif provider_instance.type == "azure":
            from infrastructure.registry.provider_registry import get_provider_registry
            from providers.azure.registration import register_azure_provider

            registry = get_provider_registry()

            register_azure_provider(registry=registry, instance_name=provider_instance.name)
            logger.debug(
                "Azure provider instance '%s' registered successfully",
                provider_instance.name,
            )
            return True
        else:
...
def _register_provider_to_context(
    ...
    try:
        ...
        if provider_type == "aws":
            ...
            return _register_azure_provider_to_context(provider_instance, provider_context, container)
        else:
            logger.warning("Unknown provider type: %s", provider_type)
            return False
...
def _register_azure_provider_to_context(
    provider_instance, provider_context: ProviderContext, container: DIContainer
) -> bool:
    """Register Azure provider instance to the provider context."""
    logger = container.get(LoggingPort)
    ...
    try:
        ...
        azure_config = AzureProviderConfig(
            provider_type="azure",
            **(provider_instance.config or {}),
        )

        def _make_azure_client():
            from providers.azure.infrastructure.azure_client import AzureClient

            class _AzureConfigPort:
                """Minimal ConfigurationPort backed by a frozen AzureProviderConfig."""
                def __init__(self, cfg: AzureProviderConfig) -> None:
                    self._cfg = cfg
                def get_typed(self, t):
                    return self._cfg if t is AzureProviderConfig else None
                def get(self, key, default=None):
                    return getattr(self._cfg, key, default)
                def get_provider_config(self):
                    return None

            return AzureClient(config=_AzureConfigPort(azure_config), logger=logger)
        ...
        provider_context.register_strategy(azure_strategy, provider_instance.name)

        logger.debug("Registered Azure provider '%s' to context", provider_instance.name)
        return True

    except Exception as e:
        logger.error(
            "Failed to register Azure provider '%s' to context: %s",
            provider_instance.name,
            str(e),
        )
        return False
