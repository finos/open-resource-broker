...
async def handle_provider_add(args) -> int:
    ...
    try:
        ...
        if args.discover:
            ...
            infrastructure_defaults = _discover_infrastructure("aws", args.aws_region, args.aws_profile)
        
        # Generate provider name
        provider_name = args.name or _generate_provider_name("aws", args.aws_profile, args.aws_region)
...
def _discover_infrastructure(provider_type: str, region: str, profile: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if hasattr(strategy, 'discover_infrastructure_interactive'):
            ...
            logger.info(f"Infrastructure discovery not supported for provider type: {provider_type}")
            return {}
