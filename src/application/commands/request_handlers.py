            ...
            if is_dry_run:
                ...
                try:
                    ...
                    if provisioning_result.get("success"):
                        ...
                        provider_data = provisioning_result.get("provider_data", {}) or {}
                        request = request.update_status(
...
class CreateReturnRequestHandler(BaseCommandHandler[CreateReturnRequestCommand, str]):
    ...
    async def _process_template_group(
        ...
        try:
            ...
            # Source them from the loaded template rather than persisted machine data.
            termination_context = {
                "correlation_id": str(request.request_id),
                "template_id": template_id,
                "parallel_execution": True,
            ...
            for key in (
                ...
                if value in (None, "") and hasattr(template, key):
                    value = getattr(template, key, None)
                if value not in (None, ""):
                    termination_context[key] = value
