...
class GetRequestHandler(BaseQueryHandler[GetRequestQuery, RequestDTO]):
    ...
    async def _fetch_provider_machines(self, request, existing_machines) -> tuple[list, dict]:
        ...
        if request.resource_ids:
            ...
            # If no machine aggregates are associated with this request (common for return requests),
            # fall back to request.instance_ids (these are internal ORB ids). Resolve them to provider
            # instance ids by looking up the machine repository.
            if not instance_ids:
                try:
                    # request.instance_ids may be list[InstanceId] or list[str]
                    req_ids = getattr(request, "instance_ids", []) or []
                    resolved = []
                    if req_ids:
                        with self.uow_factory.create_unit_of_work() as uow:
                            for rid in req_ids:
                                rid_str = str(rid) if not hasattr(rid, "value") else str(rid.value)
                                try:
                                    m = uow.machines.find_by_id(rid_str)
                                    if m and getattr(m, "instance_id", None):
                                        resolved.append(str(m.instance_id.value))
                                    else:
                                        # Assume it might already be a provider instance id
                                        resolved.append(rid_str)
                                except Exception:
                                    # If lookup fails, still include the raw id
                                    resolved.append(rid_str)
                    if resolved:
                        instance_ids = resolved
                except Exception as exc:
                    self.logger.debug("Failed to resolve request.instance_ids to provider ids: %s", exc)
        ...
        for inst in raw_instances or []:
            ...
            if hasattr(inst, "instance_id"):
                ...
                continue
            try:
                provider_machines.append(self._create_machine_from_provider_data(inst, request))
            except Exception as exc:
                self.logger.warning(
    ...
    async def _update_machine_status_from_aws(
        ...
        try:
                ...
                if existing:
                    ...
                    try:
                        ...
                        if isinstance(dm, DomainMachine):
                            ...
                            created_machine = self._create_machine_from_provider_data(
            ...
            if provider_errors:
                ...
                if request_metadata.get("fleet_errors") != provider_errors:
                    request_metadata["fleet_errors"] = provider_errors
    ...
    # TODO: Check changing this didn't break AWS
    def _create_machine_from_provider_data(self, provider_instance: dict[str, Any], request):
        """Create machine aggregate using provider-agnostic format detection."""
        from domain.base.value_objects import InstanceId
        from domain.machine.aggregate import Machine

        # Detect format and normalize to snake_case for Pydantic
        if "instance_id" in provider_instance:
            ...
            machine_data = {
                ...
                if isinstance(provider_instance.get("State"), dict)
                else provider_instance.get("State"),
                "instance_type": provider_instance.get("InstanceType"),
                "image_id": provider_instance.get("ImageId", "unknown"),
                "private_ip": provider_instance.get("PrivateIpAddress"),
                "public_ip": provider_instance.get("PublicIpAddress"),
                ...
                "tags": {
                    "tags": {
                        ...
                        for tag in provider_instance.get("Tags", [])
                    }
                },
                "metadata": provider_instance,
            }
        ...
        machine_data["instance_type"] = InstanceType(
            value=machine_data.get("instance_type") or "unknown"
        )
...
class GetTemplateHandler(BaseQueryHandler[GetTemplateQuery, Template]):
    ...
    async def execute_query(self, query: GetTemplateQuery) -> Template:
        ...
        try:
            ...
            template_data.setdefault(
                "subnet_ids",
                config.get("subnet_ids") or config.get("subnetIds") or [],
            )
            template_data.setdefault(
