...
class ProviderSelectionService:
    ...
    def _select_override_provider_for_templates(
        ...
        return ProviderSelectionResult(
            ...
            provider_instance=provider_instance.name,
    ...
    def _get_provider_instance_config(self, provider_name: str) -> Optional[ProviderInstanceConfig]:
        ...
        enabled_type_matches = [
            provider
            for provider in self._provider_config.providers
            if provider.type == provider_name and provider.enabled
        ]
        if len(enabled_type_matches) == 1:
            self._logger.debug(
                "Resolved provider alias '%s' to instance '%s'",
                provider_name,
                enabled_type_matches[0].name,
            )
            return enabled_type_matches[0]
