...
``FollowUpContext`` replaces stringly-typed ``provider_metadata`` keys such as
``termination_follow_up_pending`` with an explicit, typed descriptor that
captures *what* kind of follow-up is needed and *what* state to expect.

This lives in the application layer because it represents application-level
orchestration concerns (polling strategy, expected transitions) rather than
