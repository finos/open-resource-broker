"""Application ports for scheduler-specific operations.

The scheduler contract is split into four cohesive sub-ports so that
collaborators can depend on only the slice they need (ISP):

  * :class:`SchedulerFormattingPort` — response/display formatting.
  * :class:`SchedulerEnvironmentPort` — directory/log/scheduler-identity lookup.
  * :class:`SchedulerTemplateIOPort` — template parsing and file-path resolution.
  * :class:`SchedulerRequestParsingPort` — request parsing and request-response
    formatting.

:class:`SchedulerPort` remains as an umbrella that inherits all four, so
existing code that depends on the full contract keeps working unchanged.
"""
...
class SchedulerFormattingPort(ABC):
    """Response and display formatting for a scheduler's wire protocol."""
...
class SchedulerEnvironmentPort(ABC):
    """Directory, logging and scheduler-identity lookup for a scheduler."""
...
class SchedulerTemplateIOPort(ABC):
    """Template parsing and template file-path resolution for a scheduler."""
...
class SchedulerRequestParsingPort(ABC):
    """Request parsing and request-creation response formatting for a scheduler."""
...
class SchedulerPort(
    ...
    """Umbrella port composing the four scheduler sub-ports.

    Existing collaborators that depend on the full scheduler contract continue
    to depend on this class; new collaborators should prefer the narrowest
    sub-port they need.
    """
