"""KubernetesDeploymentHandler — ``apps/v1 Deployment`` provisioning handler.
...
# used by ``KubernetesPodHandler`` for create / delete operations.
_MAX_CONCURRENT_PATCHES = 50


@injectable
class KubernetesDeploymentHandler(KubernetesHandlerBase):
    """Handler for the ``KubernetesDeployment`` provider-API key.
    ...
    def __init__(
        ...
        *,
        ...
        # :meth:`KubernetesPodHandler._read_from_cache` for the
        # semantics.
    ...
    def get_example_templates(cls) -> list[Template]:
        """Return one example template that submits as a ``KubernetesDeployment``."""
        return [
            Template(
                template_id="kubernetes-deployment-example",
