"""KubernetesJobHandler — ``batch/v1 Job`` provisioning handler.
...
from orb.providers.kubernetes.utilities.job_spec import (
    build_job_spec,
    make_job_name,
)
...
@injectable
class KubernetesJobHandler(KubernetesHandlerBase):
    """Handler for the ``KubernetesJob`` provider-API key.
    ...
    PROVIDER_API: str = "KubernetesJob"
    ...
    def get_example_templates(cls) -> list[Template]:
        """Return one example template that submits as a ``KubernetesJob``."""
        return [
            Template(
                template_id="kubernetes-job-example",
