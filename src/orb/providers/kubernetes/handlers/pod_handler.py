"""KubernetesPodHandler — direct ``v1/Pod`` provisioning handler.
...
@injectable
class KubernetesPodHandler(KubernetesHandlerBase):
    """Handler for the ``KubernetesPod`` provider-API key.
        ...
        * ``provider_data`` — ``{"namespace": ns, "pod_names": [...]}``.
        ...
        for seq in range(count):
            ...
            pod_body = build_pod_spec(
                ...
                request,
                pod_name=pod_name,
                machine_id=pod_name,  # 1 pod = 1 machine for KubernetesPod
                namespace=namespace,
    ...
    def check_hosts_status(self, request: Request) -> CheckHostsStatusResult:
        ...
        Implementation is on-demand: a single
        ``list_namespaced_pod(label_selector=orb.io/request-id=<id>)`` is
        issued per call.  Phase C swaps this for an in-memory cache
        populated by the watch task.
        """
    ...
    async def _delete_one_pod(
        ...
        *,
        ...
        :meth:`KubernetesHandlerBase.with_retry`.
        """
        async with sem:
            try:
                await asyncio.to_thread(
    ...
    def get_example_templates(cls) -> list[Template]:
        """Return one example template that submits as a ``KubernetesPod``."""
        return [
            Template(
                template_id="kubernetes-pod-example",
