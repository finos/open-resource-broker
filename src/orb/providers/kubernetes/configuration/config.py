...
``BaseSettings`` with an ``ORB_KUBERNETES_`` env-var prefix plus a
...
class KubernetesProviderConfig(BaseSettings, BaseProviderConfig):  # type: ignore[misc]
    ...
    * ``namespaces`` — multi-namespace mode.  ``None`` falls back to
      ...
      RBAC (see ``docs/providers/kubernetes/rbac.yaml`` — added in
      Phase G).
    ...
    * ``min_kubernetes_version`` — minimum K8s API server version the
    ...
    def _validate_label_prefix(self) -> "KubernetesProviderConfig":
