    ...
    try:
        ...
        for template in templates:
            ...
            template_id = template.get(
                "template_id") or template.get("templateId", "N/A")
            name = template.get("name", "N/A")
            provider_api = template.get(
                "provider_api") or template.get("providerApi", "N/A")
            max_instances = template.get(
                "max_instances") or template.get("maxNumber", "N/A")
            ...
            if attributes and isinstance(attributes, dict):
                ...
                instance_type = template.get(
                    "instance_type") or template.get("instanceType", "N/A")
...
def _format_ascii_table(templates: List[Dict]) -> str:
    ...
    for template in templates:
        ...
        if attributes and isinstance(attributes, dict):
            # HF format - extract from attributes
            cpus = (
                attributes.get("ncpus", ["Numeric", "N/A"]
                               )[1] if attributes.get("ncpus") else "N/A"
            )
            ram = attributes.get("nram", ["Numeric", "N/A"]
                                 )[1] if attributes.get("nram") else "N/A"
        else:
            # Standard format - derive from instance_type
            instance_type = template.get(
                "instance_type") or template.get("instanceType", "N/A")
...
def format_requests_list(requests: List[Dict]) -> str:
    ...
    for i, request in enumerate(requests):
        ...
        num_requested = request.get(
            "requested_count") or request.get("numRequested", "N/A")
...
def format_machines_list(machines: List[Dict]) -> str:
    ...
    for i, machine in enumerate(machines):
        ...
        name = (
            machine.get("name") or machine.get(
                "machine_name") or machine.get("machineName", "N/A")
        )
